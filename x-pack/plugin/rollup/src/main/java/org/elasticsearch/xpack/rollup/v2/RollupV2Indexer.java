 ...
 */
public abstract class RollupV2Indexer extends AsyncTwoPhaseIndexer<Map<String, Object>, RollupIndexerJobStats> {
     ...
     */
    RollupV2Indexer(ThreadPool threadPool, String executorName, RollupV2Job job, AtomicReference<IndexerState> initialState,
    ...
    protected IterationResult<Map<String, Object>> doProcess(SearchResponse searchResponse) {
        ...
        return new IterationResult<>(processBuckets(response, job.getConfig().getRollupIndex(), getStats(),
    ...
    static List<IndexRequest> processBuckets(CompositeAggregation agg, String rollupIndex, RollupIndexerJobStats stats,
        ...
        return agg.getBuckets().stream().map(b ->{
            ...
            metrics.forEach(a -> {
                String name = a.getName().substring(0, a.getName().indexOf(".value"));
