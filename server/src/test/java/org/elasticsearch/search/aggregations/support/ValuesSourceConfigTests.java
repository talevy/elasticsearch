...
public class ValuesSourceConfigTests extends ESSingleNodeTestCase {
    ...
    public void testGeoShape() throws IOException {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            assertEquals(1, values.docValueCount());
            MultiGeoValues.GeoValue value = values.nextValue();
            MultiGeoValues.BoundingBox box = value.boundingBox();
            assertThat(box.negLeft, closeTo(-10, GeoUtils.TOLERANCE));
            assertThat(box.top, closeTo(10, GeoUtils.TOLERANCE));
        }
    }
    ...
    public void testEmptyGeoShape() throws IOException {
        ...
        try (Engine.Searcher searcher = indexService.getShard(0).acquireSearcher("test")) {
            ...
            MultiGeoValues.GeoValue value = values.nextValue();
            MultiGeoValues.BoundingBox box = value.boundingBox();
            assertThat(box.posLeft, closeTo(0, GeoUtils.TOLERANCE));
            assertThat(box.top, closeTo(0, GeoUtils.TOLERANCE));

            IllegalArgumentException exception = expectThrows(IllegalArgumentException.class,
                () -> ValuesSourceConfig.resolve(context, null, "geo_shapes", null, "invalid",
